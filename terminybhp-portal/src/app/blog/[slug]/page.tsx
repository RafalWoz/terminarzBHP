...
export async function generateMetadata({ params }: BlogPostPageProps) {
  ...
  return {
    ...
    openGraph: { title: post.title, description: post.description, images: post.ogImage ? [{ url: post.ogImage, alt: post.imageAlt || post.title }] : undefined },
  };
}
    ...
    <main>
      <section className="border-b border-[var(--slate-200)] bg-[linear-gradient(180deg,#fff,var(--paper))]">
        <div className="mx-auto grid max-w-[1160px] gap-8 px-5 py-12 sm:px-6 lg:grid-cols-[1fr_340px] lg:items-start lg:py-16">
          <article>
            <Link href="/blog" className="text-sm font-extrabold text-[var(--teal-700)] hover:text-[var(--navy-950)]">Wróć do bloga</Link>
            <div className="mt-8 inline-flex rounded-full bg-[var(--green-50)] px-3 py-1.5 text-xs font-extrabold text-[var(--teal-700)]">{post.category} · {post.readingTime} · {new Date(post.date).toLocaleDateString("pl-PL")}</div>
            ...
            {post.coverImage ? (
              <figure className="mt-8 max-w-4xl overflow-hidden rounded-[24px] border border-[var(--slate-200)] bg-white shadow-[0_8px_24px_rgba(7,24,38,0.05)]">
                <img src={post.coverImage} alt={post.imageAlt || post.title} title={post.imageTitle} className="aspect-[16/9] w-full object-cover" />
              </figure>
            ) : null}
          </article>
      ...
      <section className="mx-auto grid max-w-[1160px] gap-8 px-5 py-12 sm:px-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <article className="rounded-[28px] border border-[var(--slate-200)] bg-white p-6 shadow-[0_8px_24px_rgba(7,24,38,0.05)] sm:p-9"><ArticleContent content={post.content} /></article>
